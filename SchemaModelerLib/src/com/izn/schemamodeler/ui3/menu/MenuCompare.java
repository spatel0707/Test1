...
public class MenuCompare implements SchemaCompare {
	...
	private Map<String,List> processSchemaCompare(Context context, Map<String,Object> list1, Map<String,Object> list2)  throws Exception{
		...
		try {
			...
			for (Map.Entry<String,Object> entry : list1.entrySet())  
			{
				...
				obj1 = (Menu)listSchemaList1.get(0);
				if(list2.containsKey(sSchemaName))
				{
					listSchemaList2 = (ArrayList)list2.get(sSchemaName);
					obj2 = (Menu)listSchemaList2.get(0);
					if(UIUtil.isEqual(obj1.name,obj2.name) && UIUtil.isEqual(obj1.description,obj2.description) && UIUtil.isEqual(obj1.hidden,obj2.hidden) && UIUtil.isEqual(obj1.registryname,obj2.registryname) && UIUtil.isEqual(obj1.getHref(),obj2.getHref())&& UIUtil.isEqual(obj1.getAlt(),obj2.getAlt())&& UIUtil.isEqual(obj1.getLabel(),obj2.getLabel()) && UIUtil.isEqualStringList(obj2.getLstCommands(), obj1.getLstCommands()) && UIUtil.isEqualStringList(obj2.getLstMenu(), obj1.getLstMenu()) && UIUtil.isEqualStringMap(obj2.getLstSetting(), obj1.getLstSetting())) {
	...
	private Map<String,Object> readFromXML(Context context, String fileName)  throws Exception{
		...
		try {
			...
			for (Schema _schema : lstSchema)
			{
				...
				menu.hidden = _basicElem.getHidden();
				...
				mSchemaInfo.put(menu.name,listSchema);
			}
		} catch (UnmarshalException ume) {
			throw ume;
		} catch (Exception e) {
	...
	private void writeToXML(Context context, String compareFileName, List<Object> list) throws Exception{	
			...
			try {
				MenuXMLSchema txc = objectFactory.createComponent();
				txc.setName(sAdminType);
				txc.setType("admin");
				txc.setVersion(version);
				txc.schema = new ArrayList<MenuXMLSchema.Schema>();
				File file = null;
				...
				for (int iCount = 0; iCount < iSize; iCount++) {
					...
					_schema = (MenuXMLSchema.Schema)listSchemaList.get(1);
