...
public class WebFormCompare implements SchemaCompare {
	...
	private Map<String,List> processSchemaCompare(Context context, Map<String,Object> list1, Map<String,Object> list2)  throws Exception{
		...
		try {
			...
			for (Map.Entry<String,Object> entry : list1.entrySet())  
			{
				...
				obj1 = (WebForm)listSchemaList1.get(0);
				if(list2.containsKey(sSchemaName))
				{
					listSchemaList2 = (ArrayList)list2.get(sSchemaName);
					obj2 = (WebForm)listSchemaList2.get(0);
					if(UIUtil.isEqual(obj1.name,obj2.name) && UIUtil.isEqual(obj1.hidden,obj2.hidden) && UIUtil.isEqual(obj1.registryname,obj2.registryname) && UIUtil.isEqual(obj1.getType(),obj2.getType()) && isEqual(obj1.getLstMapColumn(), obj2.getLstMapColumn())&& UIUtil.isEqual(obj1.description,obj2.description)) {
	...
	public boolean isEqual(Map<String,WebForm.Column> list1, Map<String,WebForm.Column> list2) throws Exception{
		if(list1.size() != list2.size()){
			return false;
		}
		WebForm.Column obj1 = null;
		WebForm.Column obj2 = null;
		String sColumnName = "";
		for (Map.Entry<String,WebForm.Column> entry : list1.entrySet())  
		{
			sColumnName = entry.getKey();
			obj1 = (WebForm.Column)entry.getValue();
			if(list2.containsKey(sColumnName))
			{
				obj2 = (WebForm.Column)list2.get(sColumnName);
				if(!(UIUtil.isEqual(obj1.columnName,obj2.columnName) && UIUtil.isEqual(obj1.label,obj2.label) && UIUtil.isEqual(obj1.columnType,obj2.columnType) && UIUtil.isEqual(obj1.expression,obj2.expression) && UIUtil.isEqual(obj1.href,obj2.href) && UIUtil.isEqual(obj1.alt,obj2.alt) && UIUtil.isEqual(obj1.range,obj2.range) && UIUtil.isEqual(obj1.update,obj2.update) && UIUtil.isEqual(String.valueOf(obj1.order),String.valueOf(obj1.order)) && UIUtil.isEqualStringMap(obj1.getLstSetting(), obj2.getLstSetting())&& UIUtil.isEqualStringMap(obj1.getLstAccessDetail(), obj2.getLstAccessDetail())&& UIUtil.isEqual(String.valueOf(obj1.order),String.valueOf(obj2.order)))) {					
					return false;
				}
			} else {
				return false;
	...
	private Map<String,Object> readFromXML(Context context, String fileName)  throws Exception{
		...
		Map<String,WebForm.Column> mColumnInfo = null;
		String sColumnName = "";
		try {
			...
			while (itrSchema.hasNext()) {
				...
				mColumnInfo = new HashMap<String,WebForm.Column>();
				iColumnCounter = 0;
				...
				if (_basicElem.getType() != null) {
					sType = _basicElem.getType().trim();
				} else {
					sType = "";
				}
				webform = new WebForm(_basicElem.getName(), sDescription, _basicElem.getHidden(),
						_basicElem.getRegistryName(), sType);
				webform.hidden = _basicElem.getHidden();
				...
				webform.setLstMapColumn(mColumnInfo);
				// Sorted one ??
				webform.setWbColumn(wbColumn);
				listSchema.add(webform);
				listSchema.add(_schemaElem);
				mSchemaInfo.put(webform.name,listSchema);
			}
		} catch (UnmarshalException ume) {
			throw ume;
		} catch (Exception e) {
	...
	private void writeToXML(Context context, String compareFileName, List<Object> list) throws Exception{
		...
		try {
			...
			{	
				...
				_schema = (WebFormXMLSchema.Schema)listSchemaList.get(1);
