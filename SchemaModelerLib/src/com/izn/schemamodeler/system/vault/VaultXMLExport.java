...
public class VaultXMLExport implements  SchemaExport{
	Map _hDBInfo = new HashMap();
	String _sDBInfo = "";

	VaultXMLSchema.Object _object = null;
	VaultXMLSchema.Object.Basic _basic = null;
	VaultXMLSchema.Object.Field _field = null;
	VaultXMLSchema.Object.Field.Detail _detail = null;
	String[] fieldDetail = {"tablespace", "indexspace", "status"};
	...
	public void exportSchema(Context context, String strAdminType, List<String> lsNames, String exportPath,
		...
		try {
		    ...
		    VaultXMLSchema vxs = objectFactory.createComponent();
		    vxs.setName(strAdminType);
		    vxs.setType("systemobject");
		    vxs.setVersion(strVersion);
		    vxs.object = new ArrayList<VaultXMLSchema.Object>();
		    JAXBContext jaxbContext = JAXBContext.newInstance(new Class[] { VaultXMLSchema.class });
			...
			for (String vo : lsNames) {
				...
				_field.detail = new ArrayList<VaultXMLSchema.Object.Field.Detail>();
				...
				vxs.object.add(_object);
				if (bSchemaSeperate) {
					vo = vo.replaceAll(UIUtil.specialCharecters, "");
					file = new File(filePath + "\\" + vo + ".xml");
					jaxbMarshaller.marshal(vxs, file);
					vxs.object = new ArrayList();
				}
				if(bLogEverything) {					
