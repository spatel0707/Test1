...
public class AssociationCompare implements SchemaCompare {
	...
	private Map<String,Object> readFromXML(Context context, String fileName)  throws Exception{
		...
		try {
			...
			JAXBContext jContext = JAXBContext.newInstance(AssociationXMLSchema.class);
			Unmarshaller unMarsheller = jContext.createUnmarshaller();
			JAXBElement<AssociationXMLSchema> associationJaxbElem = unMarsheller.unmarshal((new StreamSource(new FileReader(fileName))), AssociationXMLSchema.class);
			AssociationXMLSchema associationElem = associationJaxbElem.getValue();
			List<Schema> lstSchema = associationElem.getSchema();
			...
			while (itrSchema.hasNext()) {
				...
				association = new Association(_basicElem.getName(), _basicElem.getDescription(), _basicElem.getHidden(),_basicElem.getRegistryName());
				_fieldElem = _schemaElem.getField();
				association.hidden=_basicElem.getHidden();
				...
				while (itrDetail.hasNext()) {
					...
					if (strFName.equalsIgnoreCase("definition")) {
						association.setDefinition(strFValue);
					} 
				}
				listSchema.add(association);
				listSchema.add(_schemaElem);
				mSchemaInfo.put(association.name,listSchema);
			}
		} catch (UnmarshalException ume) {
			throw ume;
		} catch (Exception e) {
	...
	private Map<String,List> processSchemaCompare(Context context, Map<String,Object> list1, Map<String,Object> list2)  throws Exception{
		...
		try {
			...
			boolean bMatch = true;
			String sSchemaName = "";
			Association obj1 = null;
			Association obj2 = null;
			...
			for (Map.Entry<String,Object> entry : list1.entrySet())  
			{
				...
				obj1 = (Association)listSchemaList1.get(0);
				if(list2.containsKey(sSchemaName))
				{
					listSchemaList2 = (ArrayList)list2.get(sSchemaName);
					obj2 = (Association)listSchemaList2.get(0);					
					if(UIUtil.isEqual(obj1.name,obj2.name) && UIUtil.isEqual(obj1.description,obj2.description) && UIUtil.isEqual(obj1.hidden,obj2.hidden) && UIUtil.isEqual(obj1.registryname,obj2.registryname) && UIUtil.isEqual(obj1.getDefinition(),obj2.getDefinition())) {
	...
	private void writeToXML(Context context, String compareFileName, List<Object> list) throws Exception{
		...
		try {
			AssociationXMLSchema txc = objectFactory.createComponent();
			txc.setName(sAdminType);
			txc.setType("admin");
			txc.setVersion(version);

			txc.schema = new ArrayList<AssociationXMLSchema.Schema>();
			File file = null;
			String filePath = "";
			...
			{	
				...
				_schema = (AssociationXMLSchema.Schema)listSchemaList.get(1);
