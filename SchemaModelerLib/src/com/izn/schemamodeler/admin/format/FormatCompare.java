...
import com.izn.schemamodeler.admin.format.Format;
import com.izn.schemamodeler.admin.format.FormatXMLSchema;
import com.izn.schemamodeler.admin.format.ObjectFactory;
...
public class FormatCompare implements SchemaCompare {
	...
	private Map<String,Object> readFromXML(Context context, String fileName)  throws Exception{
		...
		try {
			...
			JAXBContext jContext = JAXBContext.newInstance(FormatXMLSchema.class);
			Unmarshaller unMarsheller = jContext.createUnmarshaller();
			JAXBElement<FormatXMLSchema> formatJaxbElem = unMarsheller.unmarshal((new StreamSource(new FileReader(fileName))), FormatXMLSchema.class);
			FormatXMLSchema formatElem = formatJaxbElem.getValue();
			List<Schema> lstSchema = formatElem.getSchema();
			...
			while (itrSchema.hasNext()) {
				...
				format = new Format(_basicElem.getName(), _basicElem.getDescription(), _basicElem.getHidden(),_basicElem.getRegistryName());
				format.hidden = this._basicElem.getHidden();
				...
				while (itrDetail.hasNext()) {
					...
					if (strFName.equalsIgnoreCase("version")) {
						format.setVersion(strFValue.trim());
					} else if (strFName.equalsIgnoreCase("filesuffix")) {
						format.setFilesuffix(strFValue.trim());
					} else if (strFName.equalsIgnoreCase("filecreator")) {
						format.setFilecreator(strFValue.trim());
					} else if (strFName.equalsIgnoreCase("filetype")) {
						format.setFiletype(strFValue.trim());
					}
				}
				listSchema.add(format);
				listSchema.add(_schemaElem);
				mSchemaInfo.put(format.name,listSchema);
			}
		} catch (UnmarshalException ume) {
			throw ume;
		} catch (Exception e) {
	...
	private Map<String,List> processSchemaCompare(Context context, Map<String,Object> list1, Map<String,Object> list2)  throws Exception{
		...
		try {
			...
			for (Map.Entry<String,Object> entry : list1.entrySet())  
			{
				...
				obj1 = (Format)listSchemaList1.get(0);
				if(list2.containsKey(sSchemaName))
				{
					listSchemaList2 = (ArrayList)list2.get(sSchemaName);
					obj2 = (Format)listSchemaList2.get(0);
					if(UIUtil.isEqual(obj1.name,obj2.name) && UIUtil.isEqual(obj1.description,obj2.description) && UIUtil.isEqual(obj1.hidden,obj2.hidden) && UIUtil.isEqual(obj1.registryname,obj2.registryname) && UIUtil.isEqual(obj1.getVersion(),obj2.getVersion()) && UIUtil.isEqual(obj1.getFilesuffix(),obj2.getFilesuffix()) && UIUtil.isEqual(obj1.getFilecreator(),obj2.getFilecreator()) && UIUtil.isEqual(obj1.getFiletype(),obj2.getFiletype())) {
...
private void writeToXML(Context context, String compareFileName, List<Object> list) throws Exception{
		...
		try {
			FormatXMLSchema txc = objectFactory.createComponent();
			txc.setName(sAdminType);
			txc.setType("admin");
			txc.setVersion(version);

			txc.schema = new ArrayList<FormatXMLSchema.Schema>();
			File file = null;
			...
			{	
				...
				_schema = (FormatXMLSchema.Schema)listSchemaList.get(1);
